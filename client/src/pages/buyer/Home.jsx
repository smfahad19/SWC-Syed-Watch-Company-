        ...
        <div className='absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 z-20'>
          {activeSlides.map((_, i) => (
            <button
              key={i}
              onClick={() => goToSlide(i)}
              className={`h[3px] rounded-full transition-all duration-300 ${i === currentSlide ? 'bg-white w-8' : 'bg-white/30 w-4'}`}
            />
          ))}
        </div>
      ...
      <div className='max-w-6xl mx-auto px-4 py-20 grid grid-cols-1 md:grid-cols-2 gap-16 items-center'>
        ...
        <div className='grid grid-cols-2 gap-4'>
          {[
            ...
            <div key={stat.label} className='border border-gray-100 p-8 text-center hover:border-black transition'>
            ...
            {filtered.map((product) => (
              <div key={product.id} className='group'>
                <Link to={`/products/${product.id}`}>
                  <div className='bg-gray-50 aspect-square mb-3 flex items-center justify-center overflow-hidden border border-gray-100 group-hover:border-gray-300 transition'>
                    {product.images?.[0] ? (
                      <img
